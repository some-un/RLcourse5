...
if __name__ == '__main__':
    ...
    import valueIterationAgents, pilearningAgents
    a = None
    if opts.agent == 'value':
        ...
        qLearnOpts = {'gamma': opts.discount,
                      ...
                      'actionFn': actionFn}
        a = pilearningAgents.PILearningAgent(**qLearnOpts)
    ...
    try:
        if not opts.manual and opts.agent == 'value':
            ...
            display.pause()
            display.displayQValues(a, message = "Pi-VALUES AFTER "+str(opts.iters)+" ITERATIONS")
            display.pause()
    except KeyboardInterrupt:
        sys.exit(0)
    ...
    if not opts.quiet:
        if opts.manual and opts.agent == None:
            ...
            if opts.agent == 'q': displayCallback = lambda state: display.displayQValues(a, state, "CURRENT PI-VALUES")
    ...
    # DISPLAY POST-LEARNING VALUES / PI-VALUES
    if opts.agent == 'q' and not opts.manual:
        try:
            display.displayQValues(a, message = "PI-VALUES AFTER "+str(opts.episodes)+" EPISODES")
            display.pause()
